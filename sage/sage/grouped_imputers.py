...
class GroupedMarginalImputerPytorch(GroupedImputer):
    '''Marginalizing out removed features with their marginal distribution.'''
    def __init__(self, model, data, groups, remaining_features=0):
        super().__init__(model, groups, data.shape[1], remaining_features)
        self.data = data
        ...
        self.samples = len(data)
        self.num_groups = len(groups)
        self.py_groups_mat = torch.Tensor(self.groups_mat).to(data.device).float()
        if len(data) > 1024:
            warnings.warn('using {} background samples may lead to slow '
                          'runtime, consider using <= 1024'.format(
                            len(data)), RuntimeWarning)

    def py_inclusion_matrix(self, S_py):
        S_py = torch.hstack([S_py, 
            torch.zeros(size=(S_py.shape[0],1), device=S_py.device, dtype=torch.bool)])
        # S[:, -1] = self.remaining it is already zero... TODO: change for the case of other default values
        return torch.matmul(S_py, self.py_groups_mat).bool()

    def __call__(self, x, S):
        # Prepare x.
        n = len(x)
        S_py = torch.Tensor(S).to(self.data.device).float()
        S_py = self.py_inclusion_matrix(S_py)
