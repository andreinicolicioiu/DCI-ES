    ...
    def __call__(self,
        ...
        for it in range(n_loops):
            ...
            for i in range(min_coalition, max_coalition):
                time_coalition_start = time.time()
                ...
                time_coalition_end = time.time()
                if i % 10 == 1:
                    print(f'[ {i}/{max_coalition-min_coalition} ]time coalitions: {time_coalition_end - time_coalition_start}')
            # Update tracker.
            ...
            if bar and detect_convergence:
                ...
                bar.refresh()
        print(f'time loop: {time.time() - time_loop_start} s')
        if bar:
            bar.close()
